...
func (repo *repo) Update(ctx context.Context, id string, firstName, lastName, email, phone *string) error {
	...
	res := repo.db.WithContext(ctx).Model(&domain.User{}).Where("id = ?", id).Updates(values)
	if res.Error != nil {
		repo.log.Println(res.Error)
		return res.Error
	}
